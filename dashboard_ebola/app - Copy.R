...
# ── Module DHIS2 (chemin relatif depuis dashboard_ebola/) ──
...
if (file.exists(.dhis2_mod)) {
  ...
  ui_dhis2_tab <- shinydashboard::tabItem(tabName='dhis2',
    shiny::fluidRow(shiny::tags$div(style='padding:20px;',
      shiny::tags$p(paste('Module DHIS2 absent:', .dhis2_mod)))))
  server_dhis2 <- function(input,output,session) {}
}

if (file.exists(.dhis2_mod)) source(.dhis2_mod) else {
  D2_OK <- FALSE
  ui_dhis2_tab <- shinydashboard::tabItem(tabName='dhis2',
