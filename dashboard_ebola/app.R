...
if (!exists('ui_dhis2_tab')) {
  D2_OK <- FALSE
  ui_dhis2_tab <- shinydashboard::tabItem(tabName='dhis2',
    shiny::fluidRow(shiny::tags$p('Module DHIS2 absent.')))
}
if (!exists('server_dhis2')) {
...
# ── Module Operational Gap Tracker / Patient Lifeline ──
.gap_mod <- if (file.exists('app_gap_tracker_module.R')) normalizePath('app_gap_tracker_module.R', mustWork=FALSE) else normalizePath('dashboard_ebola/app_gap_tracker_module.R', mustWork=FALSE)
GAP_OK <- FALSE
GAP_ERROR <- NA_character_
if (file.exists(.gap_mod)) {
  tryCatch({
    source(.gap_mod)
    GAP_OK <- exists('ui_gap_tracker_tab') && exists('server_gap_tracker')
  }, error = function(e) {
    GAP_OK <<- FALSE
    GAP_ERROR <<- conditionMessage(e)
  })
} else {
  GAP_OK <- FALSE
  GAP_ERROR <- paste('Module gap tracker absent:', .gap_mod)
}
  ...
  ui_gap_tracker_tab <- shinydashboard::tabItem(
    ...
    shiny::fluidRow(
      shinydashboard::box(
        ...
        title = 'Operational Gap Tracker',
        shiny::tags$p('Module Operational Gap Tracker non disponible ou non chargé.'),
        shiny::tags$p(GAP_ERROR)
      )
    )
  )
}
if (!exists('server_gap_tracker')) {
  server_gap_tracker <- function(input, output, session) {}
}
...
# find_first : prend le premier chemin/URL utilisable.
# PRINCIPE : les DONNEES DYNAMIQUES (serie, zones, indicateurs) qui changent
# a chaque SitRep doivent venir de GitHub en PRIORITE, sinon un vieux fichier
# embarque dans le deploiement shinyapps masque les donnees fraiches (bug
# classique : dashboard fige sur un ancien SitRep). On expose donc une
# fonction prefere_github() qui met l'URL GitHub en tete pour ces fichiers.
# Les donnees STATIQUES (geojson, qui ne changent pas) gardent la priorite
...
ui <- dashboardPage(
  ...
  dashboardBody(
    tags$head(tags$style(HTML("
      /* === CHARTE AFRICA CDC / UNION AFRICAINE === */
      .skin-red .main-header .logo {background:#0A3D2E!important;font-weight:600;color:#FCD116!important;}
      .skin-red .main-header .navbar {background:#007A3D!important;}
      .skin-red .main-sidebar {background-color:#0A3D2E!important;}
      .skin-red .sidebar a {color:#E8F0EC!important;}
      .skin-red .sidebar-menu>li.active>a {background:#007A3D!important;border-left:3px solid #FCD116!important;color:#fff!important;}
      .skin-red .sidebar-menu>li:hover>a {background:#0F4D3A!important;border-left:3px solid #FCD116!important;}
      .content-wrapper,.right-side {background-color:#F4F6F5!important;}
      .small-box {border-radius:10px!important;box-shadow:0 2px 6px rgba(0,0,0,0.12)!important;min-height:118px!important;}
      .small-box h3 {font-size:30px!important;font-weight:700!important;}
      .small-box.bg-red {background:#CE1126!important;}
      .small-box.bg-green {background:#007A3D!important;}
      .small-box.bg-yellow {background:#FCD116!important;color:#5A4A00!important;}
      .small-box.bg-yellow .icon-large,.small-box.bg-yellow p {color:#5A4A00!important;}
      .small-box.bg-orange {background:#E8841A!important;}
      .small-box.bg-navy {background:#0A3D2E!important;}
      .small-box.bg-black {background:#1A1A1A!important;}
      .box {border-radius:10px;box-shadow:0 2px 6px rgba(0,0,0,0.08)!important;}
      .box.box-solid.box-primary>.box-header {background:#007A3D!important;}
      .box.box-primary {border-top-color:#007A3D!important;}
      .box.box-solid.box-danger>.box-header {background:#CE1126!important;}
      .box.box-danger {border-top-color:#CE1126!important;}
      .box.box-solid.box-warning>.box-header {background:#E8841A!important;}
      .box.box-warning {border-top-color:#FCD116!important;}
      .box.box-solid.box-info>.box-header {background:#0A3D2E!important;}
      .box-title {font-size:15px!important;font-weight:600!important;}
      .js-plotly-plot .xtick text,.js-plotly-plot .ytick text {font-size:12px!important;fill:#0A3D2E!important;}
      table.dataTable thead th {background:#0A3D2E!important;color:#fff!important;}
...
server <- function(input, output, session) {
  ...
  output$dynamic_menu <- renderMenu({
    sidebarMenu(
      ...
      ,menuItem("DHIS2 Situation Room", tabName = "dhis2",
               icon = icon("database"),
  ...
  # Module Operational Gap Tracker
  if (exists('server_gap_tracker')) server_gap_tracker(input, output, session)
